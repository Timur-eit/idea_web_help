...
describe('Cursor tests', () => {
	...
	const myCursor = (currentId, activePages) => new Cursor(pageList, pages, currentId, activePages)
  	...
  	topLevelIds.map((topId, i) => {
  	  	it('Cursor`s getDownDirection has worked: for non-nested Ids', () => {
  	  	  expect(i === topLevelIds.length - 1 ? topLevelIds[0] : topLevelIds[i + 1])
			  .toBe(myCursor(topId, []).getDownDirection())
  	  	})
  	  	it('Cursor`s getUpDirection have worked: for non-nested Ids', () => {
  	  	  expect(i === 0 ? topLevelIds[topLevelIds.length - 1] : topLevelIds[i - 1])
			  .toBe(myCursor(topId, []).getUpDirection())
  	  	})
  	  	...
  	  	it('Cursor`s getDownDirection has worked: for nested Ids', () => {
  	  	  ...
  	  	  expect(getExpectedValueDown(topId, i))
			  .toBe(myCursor(topId, activePages).getDownDirection())
  	  	})
  	})
	...
	it('Cursor`s getDownDirection and getUpDirection have worked: for one exact Id', () => {
		...
		expect('preferences.jdks')
			.toBe(myCursor('Unloading_Modules', thisCaseActivePages).getDownDirection())
		expect('Unloading_Modules')
			.toBe(myCursor('preferences.jdks', thisCaseActivePages).getUpDirection())

		expect('Working_with_source_code')
			.toBe(myCursor('concepts.module.libraries', thisCaseActivePages).getDownDirection())
		expect('concepts.module.libraries')
			.toBe(myCursor('Working_with_source_code', thisCaseActivePages).getUpDirection())
	...
	it('Cursor getMoveCursorRight have worked', () => {
		...
		const myCursorRightHandler = () => {
			const cursor = new ArrowKeysHandler(pageList, pages, 'top', imitationStoreData.activePages, setCurrentId, setActivePage, isNested)
			cursor.getMoveCursorRight()
			return {currentId: imitationStoreData.currentId, activePages: imitationStoreData.activePages}
		}

		expect('Install_and_set_up__product_')
			.toBe(myCursorRightHandler().currentId)
		expect('top')
			.toBe(myCursorRightHandler().activePages[0])
